...
@synthesize editBarButtonItem=_editBarButtonItem, aboutBarButtonItem=_aboutBarButtonItem;
...
- (void)viewDidLoad {
    ...
    self.aboutBarButtonItem.title = NSLocalizedString(@"About", @"tab bar item");
    ...
    _noMediaViewController = [[MVLCNoMediaViewController alloc] initWithNibName:@"MVLCNoMediaView" bundle:nil];
...
- (void)_setEditMode:(BOOL)editMode {
    ...
    if (editMode) {
        ...
        self.editBarButtonItem.title = NSLocalizedString(@"Done", @"edit bar button item");
    } else {
        self.editBarButtonItem.style = UIBarButtonItemStylePlain;
        self.editBarButtonItem.title = NSLocalizedString(@"Edit", @"edit bar button item");
    }
}
...
- (void)_presentNoMediaViewControllerIfNeeded {
    if ([_allMedia count] == 0 && _noMediaViewController) { // Checking for _noMediaViewController is important because on load it might be nil
        if ([self presentedViewController] != _noMediaViewController)
        {
            // [self presentViewController:_noMediaViewController animated:NO completion:NULL];
            NSURL * urlToOpen = [NSURL URLWithString:@"rtsp://218.87.254.137:554/0/888888:888888/2sub"];
